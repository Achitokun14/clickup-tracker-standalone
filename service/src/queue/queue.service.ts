...
export class QueueService implements OnModuleInit, OnModuleDestroy {
	...
	async onModuleInit(): Promise<void> {
		this.log.log(
			`queue init (redis ${this.connection.host}:${this.connection.port})`,
		);
	}
	...
	registerQueue(name: string, handler: JobHandler): void {
		...
		try {
			...
			const worker = new Worker(
				...
				async (job) => {
					try {
						...
						this.log.error(
							`job ${name}:${job.id} failed: ${(err as Error).message}`,
						);
						throw err;
					}
				},
			...
			worker.on("completed", (job) =>
				this.log.debug(`job ${name}:${job.id} ok`),
			);
			...
			this.degraded = true;
			this.log.warn(
				`queue "${name}" unavailable; inline mode: ${(err as Error).message}`,
			);
		}
	}
	...
	async addJob(
		...
		if (!queue) {
			if (handler) {
				try {
					...
					this.log.warn(
						`inline ${queueName} handler failed: ${(err as Error).message}`,
					);
				}
			}
			return;
		...
		try {
			// BullMQ rejects ':' in jobId (treated as a hash separator). All our
			// upstream callers compose jobIds like "backfill:<uuid>" — sanitize
			// here so the queue accepts them instead of falling through to inline.
			const safeJobId = opts?.jobId?.replace(/:/g, "_");
			...
			this.log.warn(
				`addJob ${queueName} fell through to inline: ${(err as Error).message}`,
			);
			if (handler) {
				try {
					await handler({ data } as Job);
