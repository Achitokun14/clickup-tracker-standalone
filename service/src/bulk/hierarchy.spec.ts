import { planRepo, LIST_NAMES, CUSTOM_FIELDS, TASK_SOURCE } from './hierarchy';
...
describe('planRepo', () => {
  ...
  it('emits one Open Work task per state entry, keyed by source:bucket:index', () => {
    const plan = planRepo(
      ...
      makeExt({
        stateEntries: [
          { source: 'STATE.json', bucket: 'pending', index: 0, preview: 'Do thing', raw: { x: 1 } },
          { source: 'STATE.json', bucket: 'next', index: 0, preview: 'Next thing', raw: { y: 2 } },
        ],
      }),
    );
