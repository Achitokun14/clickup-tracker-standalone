...
import { ClickUpDirectService, ClickUpTaskFull } from '../clickup/clickup-direct.service';
...
export class BackupService {
  ...
  async take(projectId: string, trigger: BackupTrigger = 'manual'): Promise<BackupRecord> {
    const project = await this.loadProject(projectId);
    if (!project) throw new NotFoundException('project not found');

    const creds = await this.credentials.forOrg(project.organisation_id);

    const folder = await this.clickup.getFolder(project.clickup_folder_id, creds.token);
    ...
    for (const [key, listId] of Object.entries(project.list_ids)) {
      try {
        const listTasks = await this.clickup.listTasksInList(listId, creds.token);
        listEntries.push({ id: listId, key, name: this.deriveListName(listTasks, key) });
        ...
        for (const t of listTasks) {
          tasks.push({
            ...
            name: t.name,
            markdown_description: t.markdown_description ?? t.description ?? t.text_content ?? '',
            status: t.status?.status ?? 'open',
          });
        }
      } catch (err) {
        this.log.warn(`snapshot list ${key} (${listId}) failed: ${(err as Error).message}`);
      }
    }
    ...
    const inserted = await this.prisma.$queryRawUnsafe<Array<{ id: string; taken_at: Date }>>(
  ...
  async list(projectId: string): Promise<BackupRecord[]> {
    const rows = await this.prisma.$queryRawUnsafe<
      Array<{ id: string; project_id: string; trigger: BackupTrigger; taken_at: Date; snapshot: BackupSnapshot }>
    >(
  ...
  async restore(
    ...
    const snapshotRow = await this.prisma.$queryRawUnsafe<Array<{ snapshot: BackupSnapshot }>>(
      `SELECT snapshot FROM clickup_tracker.backups
       WHERE id = $1::uuid AND project_id = $2::uuid`,
      backupId,
      projectId,
    );
    if (snapshotRow.length === 0) throw new NotFoundException('backup not found');
    ...
    for (const list of snapshot.lists) {
      try {
        liveTasksByListId.set(list.id, await this.clickup.listTasksInList(list.id, creds.token));
      } catch {
      ...
      if (!live) {
        ...
        try {
          const fresh = await this.clickup.createTask(
            snapTask.list_id,
            { name: snapTask.name, markdown_description: snapTask.markdown_description },
            creds.token,
          );
        ...
        const liveDesc = live.markdown_description ?? live.description ?? live.text_content ?? '';
        if (live.name !== snapTask.name || liveDesc.trim() !== snapTask.markdown_description.trim()) {
          try {
            ...
            this.log.warn(`restore updateTask failed: ${(err as Error).message}`);
            skipped++;
          }
        } else {
          skipped++;
  ...
  private async loadProject(projectId: string): Promise<ProjectForBackup | null> {
